...
def minOverallAwkwardness(arr):
  ...
  guests = [ (ix,x) for ix,x in enumerate(arr) ]
  seats = [ 0 for ix in range(len(arr)) ]
  guests.sort(key = lambda x:x[1])
  n = len(arr)
  diff = 0
  head = 0
  tail = n-1
  while head < tail: # len(guests) > 1:
    guest1 = guests.pop()
    # seats[head] = guest1 # (index,height)
    seats[head] = guest1[0]
    head += 1
    try:
        guest2 = guests.pop()
        # seats[tail] = guest2 # (index,height)
        seats[tail] = guest2[0]
        tail -= 1
    except:
        pass
  ...
  maxdiff = 0
  for ix in range(n):
    diff = abs(arr[seats[ix]] - arr[seats[(ix+1)%n]])
    maxdiff = max(diff, maxdiff)
  return diff

minOverallAwkwardness([5, 10, 6, 8])
