...
At a given time t, measured in days, the number of users using an app is g^t (for simplicity we will allow fractional users),
...
def getBillionUsersDay(growthRates):
  ...
  threshold = OneBillion
  n = len(growthRates)
  mx = max(growthRates)
  est = (math.log(threshold) - n) / math.log(mx)
  growth = [ 1 for g in growthRates ]
  t = 0
  total = 0
  while total < threshold:
    growth = [ g*x for g,x in zip(growthRates,growth) ]
    total = sum(growth)
    t += 1
