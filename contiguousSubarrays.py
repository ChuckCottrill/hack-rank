...
def count_subarrays_brute(arr):
  ...
  for ix,x in enumerate(arr):
    ...
    for direction in [1, -1]:
      step = 1
      while (0 <= ix+ direction*step < n) and (arr[ix+ direction*step] < x):
        result[ix] += 1
        step += 1
  return result
...
def count_subarrays(arr):
  ...
  for ix,x in enumerate(arr):
    while stack and x >= arr[stack[-1]]:
      print(n-ix)
      print(stack[-1])
      result[stack.pop()] -= n-ix
    stack.append(ix)
  # clear stack
  stack.clear()
  # reversed (from R to L)
  for ix,x in reversed(list(enumerate(arr))):
    while stack and x >= arr[stack[-1]]:
      print(n-ix)
      print(stack[-1])
      result[stack.pop()] -= ix+1
    stack.append(ix)
  return result
